...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $stmt = $pdo->prepare('UPDATE barang1 SET id = ?, nama = ?, pengadaan = ?, terpakai = ?, rusak = ?, sisa = ? WHERE id = ?');
    ...
    $stmt = $pdo->prepare('SELECT * FROM barang1 WHERE id = ?');
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$brg['id']?>" method="post">
        ...
        <input type="number" name="sisa" value="<?=$brg['sisa']?>" id="sisa">
